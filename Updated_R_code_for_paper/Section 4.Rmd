...
setwd("/Volumes/Elements/Inhibitor_paper/Section4")
...
B_Subset_sig_OTU_T1_genus_edited=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/B_sigOTU_T1_edited.csv")
...
#read file response columns added to it,edit Proteobacteria col in B_All_sig_OTU_Gen_T1_df to Alpha,Beta,Delta and Gamma> use silva to charcterize, manually add columns for response to urea and response to inhibitors
B_T1_growth_response=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/B_All_sig_OTU_Gen_T1_df_edited.csv")
...
BT1_Positive_Stimulated_plot=ggplot(BT1_Positive_Stimulated, 
  ...
  ylab("Abundance")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=20),strip.text = element_text(size=20),axis.text.y = element_text(size=20), axis.title=element_text(size=25,face="bold"),legend.text=element_text(size=25),legend.key.size=unit(0.75,"cm"),legend.title=element_text(size=25,face="bold"))+
...
BT1_Positive_Stimulated_plot=BT1_Positive_Stimulated_plot+scale_x_discrete(labels=c("Negative Control" = "NC", "Positive Control" = "PC","Antibacterial" = "AB","Antifungal"="AF","Antibacterial+Antifungal"="AB+AF"))

#BT1_Positive_Stimulated_plot=BT1_Positive_Stimulated_plot+ theme(legend.position = "none")
BT1_Positive_Stimulated_plot

#ggsave("/Volumes/Elements/Inhibitor_paper/Section4/Fig5PS.pdf",width=12,height=10,units ="cm", device="pdf")
#Export landscape , height= 13.34 width =26.26
#BT1_Positive_Stimulated_plot=BT1_Positive_Stimulated_plot+ theme(legend.position = "none")
#BT1_Positive_Stimulated_plot
```
...
#Rename x tick mark labels
BT1_Positive_Inhibited_plot=BT1_Positive_Inhibited_plot+scale_x_discrete(labels=c("Negative Control" = "NC", "Positive Control" = "PC","Antibacterial" = "AB","Antifungal"="AF","Antibacterial+Antifungal"="AB+AF"))
BT1_Positive_Inhibited_plot

#BT1_Positive_Inhibited_plot=BT1_Positive_Inhibited_plot + theme(legend.position = "none")
#BT1_Positive_Inhibited_plot
```
...
```{r}
#write csv for Negative Stimulated to remove Oceanobacillus that does not have a count
...
#Rename x tick mark labels
BT1_Negative_Stimulated_plot=BT1_Negative_Stimulated_plot+scale_x_discrete(labels=c("Negative Control" = "NC", "Positive Control" = "PC","Antibacterial" = "AB","Antifungal"="AF","Antibacterial+Antifungal"="AB+AF"))

#BT1_Negative_Stimulated_plot=BT1_Negative_Stimulated_plot+ theme(legend.position = "none")
BT1_Negative_Stimulated_plot

#BT1_Negative_Stimulated_plot=BT1_Negative_Stimulated_plot+scale_fill_manual(values=c("#800026","#BD0026","#FC4E2A","#FD8D3C","#FED976","#E6AB02","#A6761D","#1D91C0","#253494","#081D58","#7FCDBB"), name="Phylum",labels=c("Alphaproteobacteria","Actinobacteria","Bacteroidetes","Betaproteobacteria","Deltaproteobacteria","Firmicutes","Gemmatimonadetes","Acidobacteria","Gammaproteobacteria","Verrucomicrobia","Planctomycetes"))
#BT1_Negative_Stimulated_plot
...
```{r}
library(ggpubr)
combined_B_OTU_D9=ggarrange(BT1_Positive_Stimulated_plot,BT1_Positive_Inhibited_plot,BT1_Negative_Stimulated_plot, ncol=3, nrow=1, widths =c(10,5,10)) #this is the the plot for the labels
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/Fig5test.pdf",width=28,height=17,units ="cm", device="pdf") # 18 and 12 is good  #use this if landscape plot  # 16 and 10 is good

#this is just the plots after remiving labels edit in inkscape later
combined_B_OTU_D92=ggarrange(BT1_Positive_Stimulated_plot,BT1_Positive_Inhibited_plot,BT1_Negative_Stimulated_plot, ncol=2, nrow=2, heights =c(10,6)) #this is the the plot for the labels  #10 4 is good for heights but you cant see the bottom plot
ggsave("/Volumes/S_ GANASAMU/Paper 3/Section 4/Fig5test1.pdf",width=28,height=17,units ="cm", device="pdf") # 18 and 12 is good  #use this if landscape plot  # 16 and 10 is good
...
#Write csv to edit names in excel, remove first OTU col
write.csv(tax_table(F_Subset_sig_OTU_T1_genus),file="F_sigOTU_T1.csv")

#import csv with changed taxa names
F_Subset_sig_OTU_T1_genus_edited=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/F_sigOTU_T1_edited.csv")
...
#write csv for B_All_sig_OTU_Gen_T1_df and manually add columns for response towards urea and response towards inhibitors 
...
F_T1_growth_response=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/F_All_sig_OTU_Gen_T1_df_edited.csv")
...
FungiGrowthrescolourlistT1 <- c(unc_Ascobolaceae="#084081",Brachyconidiellopsis="#800026",unc_Helotiales="#BD0026",unc_Mortierellales="#0868AC",Nectria="#FC4E2A")

FungiGrowthrescolourlistT5 <- c(unc_Agaricomycetes="#E31A1C",unc_Tremellales="#2B8CBE")

FungiGrowthrescolourlistT6 <- c(unc_LKM11="#662506",unc_Pleosporales="#993404",unc_Tremellales="#CC4C02",Tapinella="#4EB3D3",Tremella="#A8DDB5",unc_Helotiales="#EC7014")
...
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/FT1_PS.pdf",width=12,height=3)
```
...
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/FT1_NS.pdf",width=9,height=3)
```
...
#read csv with edited names
#import csv with changed taxa names, have to import manually no idea why read csv does not work
F_Subset_sig_OTU_T5_genus_edited=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/F_sigOTU_T5_edited.csv")

#Edit row names so that it is in the same format as taxonomy table in the phyloseq obj otherwise can't merge
row.names(F_Subset_sig_OTU_T5_genus_edited)<-F_Subset_sig_OTU_T5_genus_edited$OTU

#create matrix from edited list
FT5=as.matrix(F_Subset_sig_OTU_T5_genus_edited)
taxFT5=tax_table(FT5) #create taxonomy table with edited genus names
tax_table(F_Subset_sig_OTU_T5_genus)=taxFT5 #change taxonomy table in original phyloseq with edited taxonomy table
...
#write csv for and manually add columns for response to urea and response towards inhibitors, remove first col with OTU
...
F_T5_growth_response=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/F_All_sig_OTU_Gen_T5_df_edited.csv")
...
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/FT5_NS.pdf",width=6,height=3)
...
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/FT5_PI.pdf",width=6,height=3)
```
...
#read csv
F_Subset_sig_OTU_T6_genus_edited=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/F_sigOTU_T6_edited.csv")

#Edit row names so that it is in the same format as taxonomy table in the phyloseq obj otherwise can't merge
row.names(F_Subset_sig_OTU_T6_genus_edited)<-F_Subset_sig_OTU_T6_genus_edited$OTU

#create matrix from edited list
FT6=as.matrix(F_Subset_sig_OTU_T6_genus_edited)
taxFT6=tax_table(FT6) #create taxonomy table with edited genus names
tax_table(F_Subset_sig_OTU_T6_genus)=taxFT6 #change taxonomy table in original phyloseq with edited taxonomy table
...
F_T6_growth_response=read.csv("/Volumes/Elements/Inhibitor_paper/Section4/F_All_sig_OTU_Gen_T6_df_edited.csv")
...
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/FT6_PS.pdf",width=9,height=3)
```
...
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/FT6_PI.pdf",width=9,height=3)
```
...
ggsave("/Volumes/Elements/Inhibitor_paper/Section4/FT6_Ni.pdf",width=9,height=3)
```
