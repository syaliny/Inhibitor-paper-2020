...
setwd("/Volumes/Elements/Inhibitor_paper/Section1")
#Bacteria
...
#Plot baseplot
plot_ordination(Bacteria_phyloseq, Bact_NMDSord, type = "samples", color = "Day", shape="Treatment") + geom_point(size = 0.3) + geom_jitter()
baseplot = plot_ordination(Bacteria_phyloseq, Bact_NMDSord, type = "samples", color = "Day",shape="Treatment") + geom_point(size = 0.3)+ geom_jitter()
baseplot

my_grob = grobTree(textGrob("Stress = 0.09", x=0.2,  y=0.95, hjust=0,gp=gpar(col="Black", fontsize=7, fontface="italic")))

#Plot
NMDS_B_Clr_grdnt = baseplot +geom_point(size = 0.3) + 
theme(axis.title.x = element_text(face="bold",size=7),
axis.text.x = element_text(angle=0, colour = "black", vjust=1, hjust = 1, size=5),
...
strip.background = element_rect(colour="black")) + annotation_custom(my_grob) + scale_shape_manual(values=c(3, 7, 19,15,17))+scale_size_manual(values=c(1,1,1,1,1))
...
NMDS_B_CG=NMDS_B_CG + scale_color_gradient(low = "#C6DBEF", high = "#08306B") 
NMDS_B_CG

#pdf("NMDSall_prokaryotes.pdf", height = 8, width = 12)
#NMDS_B_CG
#dev.off()
...
#ggsave("/Volumes/S_ GANASAMU/Paper 3/Section 1/FigS1_B.pdf",width=8,height=6,units ="cm", device="pdf") # 18 and 12 is good  #use this if landscape plot  # 16 and 10 is good

#pdf("Stressplotall_prokaryotes.pdf", height = 8, width = 12)
#Stressplotall_prokaryotes
#dev.off()
...
#Plot
Bact_NMDS_ordell_plot<-ggplot(data = Bact_NMDS, aes(MDS1, MDS2)) + geom_point(size=1,aes(color = Treatment)) +
  geom_path(data=Bact_ordell_df, aes(x=NMDS1, y=NMDS2,colour=Treatment), size=0.5, linetype=2)+
  theme_bw() +
  theme(
  ...
  theme(axis.title.x = element_text(face="bold",size=7),
        axis.text.x = element_text(colour = "black", vjust=1, hjust = 1, size=5),
        axis.text.y = element_text(colour = "black", size=5),
        axis.title.y = element_text(face="bold", size=7),
        legend.title = element_text(size=7,face="bold"),
        legend.text = element_text(size = 7),
...
#Save table as csv
write.csv(Bact_all_adonis_df, file ="/Volumes/Elements/Inhibitor_paper/Section1/Bact_all_adonis_table.csv")
```
...
#Save table as csv
write.csv(Bact_noneg_adonis, file ="/Volumes/Elements/Inhibitor_paper/Section1/Bact_noneg_adonis_table.csv")
```
...
#Plot baseplot
plot_ordination(Fungi_phyloseq, Fungi_NMDSord, type = "samples", color = "Day", shape="Treatment") + geom_point(size = 0.3) + geom_jitter()
baseplot2 = plot_ordination(Fungi_phyloseq, Fungi_NMDSord, type = "samples", color = "Day",shape="Treatment") + geom_point(size = 0.3)+ geom_jitter()
baseplot2

my_grob2 = grobTree(textGrob("Stress = 0.21", x=0.3,  y=0.95, hjust=0,gp=gpar(col="Black", fontsize=7, fontface="italic")))

#Plot
NMDS_F_Clr_grdnt = baseplot2 +geom_point(size = 0.3) + 
theme(axis.title.x = element_text(face="bold",size=7),
axis.text.x = element_text(angle=0, colour = "black", vjust=1, hjust = 1, size=5),
axis.text.y = element_text(colour = "black", size=5), 
axis.title.y = element_text(face="bold",size=7),
...
panel.background = element_blank(),
panel.border = element_rect(fill = NA, colour = "black"),
strip.background = element_rect(colour="black")) + annotation_custom(my_grob2) + scale_shape_manual(values=c(3, 7, 19,15,17))+scale_size_manual(values=c(1,1,1,1,1))
...
NMDS_F_CG=NMDS_F_CG + scale_color_gradient(low = "#E5F5E0", high = "#00441B") 
NMDS_F_CG
...
##ggarrange NMDS plots for all treatments
```{r}
library(ggpubr)
combined_NMDS_CG_BF=ggarrange(NMDS_B_CG,NMDS_F_CG, ncol=2, nrow=1)
combined_NMDS_CG_BF

ggsave("/Volumes/Elements/Inhibitor_paper/Section1/Fig1.pdf",width=18,height=6,units ="cm", device="pdf") # 18 and 12 is good  #use this if landscape plot  # 16 and 10 is good


#pdf("Fig1.pdf", height = 8, width = 22)
#combined_NMDS_CG_BF
#dev.off()

```

#ggarrange stressplot all treatments
```{r}
library(ggpubr)
#combined_stressplot_all_BF=ggarrange(Stressplotall_prokaryotes,Stressplotall_fungi, ncol=2, nrow=1)
#combined_stressplot_all_BF

#ggsave("/Volumes/S_ GANASAMU/Paper 3/Section 1/Fig1_1.pdf",width=17,height=6,units ="cm", device="pdf") # 18 and 12 is good  #use this if landscape plot  # 16 and 10 is good
...
#Plot
Fungi_NMDS_ordell_plot<-ggplot(data = Fungi_NMDS, aes(MDS1, MDS2)) + geom_point(size=1,aes(color = Treatment)) +
  geom_path(data=Fungi_ordell_df, aes(x=NMDS1, y=NMDS2,colour=Treatment), size=0.5, linetype=2)+
  theme_bw() +
  theme(
    plot.background = element_blank()
    ,panel.grid.major = element_blank()
...
##ggarrange NMDS plots for urea samples only
```{r}
library(ggpubr)
combined_ellipse_CG_ureaBF=ggarrange(Bact_NMDS_ordell_plot,Fungi_NMDS_ordell_plot, ncol=2,nrow=1, widths  = c(1.55, 2.5))
combined_ellipse_CG_ureaBF

ggsave("/Volumes/Elements/Inhibitor_paper/Section1/FigS2.pdf",width=17,height=8,units ="cm", device="pdf") # 18 and 12 is good  #use this if landscape plot  # 16 and 10 is good


#pdf("FigS2.pdf", height = 8, width = 22)
#combined_ellipse_CG_ureaBF
#dev.off()
...
#Save table as csv
write.csv(Fungi_all_adonis, file ="/Volumes/Elements/Inhibitor_paper/Section1/Fungi_all_adonis_table.csv")
```
...
#Save table as csv
write.csv(Fungi_noneg_adonis, file ="/Volumes/Elements/Inhibitor_paper/Section1/Fungi_noneg_adonis_table.csv")
```
...
colnames(Bact_wu.m)[1] <- "Var1" 
  ...
  #this does not work whyyyy
  # get sample data (S4 error OK and expected)
  Bact_sd = data.frame(sample_data(p)) %>%
   select(s, d,t) %>%
    mutate_if(is.factor,as.character) 
...
colnames(Fungi_wu.m)[1] <- "Var1" 
  ...
  # get sample data (S4 error OK and expected)
  Fungi_sd = data.frame(sample_data(p)) %>%
    select(s,d,t) %>%
    mutate_if(is.factor,as.character) 
...
#Plot
Combined_lg=ggplot(SummSE_Bactfungi,aes(x=Day,y=Bray_dissimilarity,group=Organism,colour=Organism))+geom_line(lwd=0.5)+
  geom_point(size=1)+geom_errorbar(aes(ymin=Bray_dissimilarity-se,ymax=Bray_dissimilarity+se),colour="black",width=0.25)+facet_wrap(~ Treatment.y, scales = "free_y",ncol=2)+theme_bw()+theme(axis.title.x = element_text(face="bold",size=6),
        axis.text.x = element_text(colour = "black", vjust=1, hjust = 1, size=5, angle=45),
        axis.text.y = element_text(colour = "black", size=5),
...
Combined_lg=Combined_lg + labs(y="Bray dissimilarity") + scale_colour_manual(values=c("#2171B5","#41AB5D"))
Combined_lg

ggsave("/Volumes/Elements/Inhibitor_paper/Section1/Fig2.pdf",width=12,height=8,units ="cm", device="pdf") # 18 and 12 is good  #use this if landscape plot  # 16 and 10 is good


#pdf("Fig2.pdf", height = 8, width = 16)
#Combined_lg
#dev.off()
```
...
#write csv
write.csv(Posthoc_Bact_lg$res, file = "/Volumes/Elements/Inhibitor_paper/Section1/PosthocDunntst_Bact_lg.csv")
...
#write csv
write.csv(Posthoc_Fungi_lg$res, file = "/Volumes/Elements/Inhibitor_paper/Section1/PosthocDunntst_Fungi_lg.csv")
