...
setwd("/Volumes/S_ GANASAMU/Paper 3/Section 5")
...
write.csv(B_T6T7otutable_df, file ="/Volumes/S_ GANASAMU/Paper 3/Section 5/B_T6T7otu_table.csv")
B_T6T7otu_table_edited = read.csv("/Volumes/S_ GANASAMU/Paper 3/Section 5/B_T6T7otu_table_edited.csv", header = TRUE)
...
#Plot
B_T6T7Heatmap_log10<-amp_heatmap(B_T6T7Heatmap_amp,facet_by = "Treatment",group_by = "Day",tax_aggregate = "Genus",tax_show = 30,plot_values = TRUE,plot_values_size = 12,
...
theme(axis.text.x = element_text(angle = 45, size=30, vjust = 1),axis.text.y = element_text(size=30),legend.title =element_text(size = 30,face="bold"),legend.text = element_text(size = 30),legend.key.size = unit(3.00, "cm"),strip.text.x = element_text(size=30, face="bold"),legend.position="right")

#Plot T6 and T7 separately
B_T6T7Heatmap_log10 + scale_x_discrete(limits = c("42")) 

#T7 separately
B_T6T7Heatmap_log10 + scale_x_discrete(limits = c("51"))
...
write.csv(F_T6T7otutable_df, file ="/Volumes/S_ GANASAMU/Paper 3/Section 5/F_T6T7otu_table.csv",row.names = F)
F_T6T7otu_table_edited = read.csv("/Volumes/S_ GANASAMU/Paper 3/Section 5/F_T6T7otu_table_edited.csv",header=T,check.names=F)
...
#Plot
F_T6T7Heatmap_log10<-amp_heatmap(F_T6T7Heatmap_amp,facet_by = "Treatment",group_by = "Day",tax_aggregate = "Genus",tax_show = 30,plot_values = TRUE,plot_values_size =12,
plot_colorscale = "log10",color_vector = rev(c("#440154FF","#482878FF","#3E4A89FF","#31688EFF","#26828EFF","#1F9E89FF","#35B779FF","#6DCD59FF","#B4DE2CFF","#FDE725FF")),plot_legendbreaks = c(0.1, 1, 5, 10,25, 50, 75)) +
theme(axis.text.x = element_text(angle = 45, size=35, vjust = 1),axis.text.y = element_text(size=35),legend.title =element_text(size = 35,face="bold"),legend.text = element_text(size = 35),legend.key.size= unit(3.20, "cm"),strip.text.x = element_text(size=35, face="bold"),legend.position="right")

#Plot T6 and T7 separately
#T6
F_T6T7Heatmap_log10 + scale_x_discrete(limits = c("42"))


#T7 separately
F_T6T7Heatmap_log10 + scale_x_discrete(limits = c("51"))
...
write.csv(SummN20_D42,file="/Volumes/S_ GANASAMU/Paper 3/Section 5/SummN20_D41.csv")
```
...
#write csv
write.csv(Posthoc_D42_N20flux$p.value, file = "/Volumes/S_ GANASAMU/Paper 3/Section 5/Posthoc_D42_N20flux.csv")
...
write.csv(SummN20_D51,file="/Volumes/S_ GANASAMU/Paper 3/Section 5/SummN20_D51.csv")
```
...
#Plot
N20_D51plot<-ggplot()+geom_errorbar(data=subset(SummN20_D51,Treatment %in% c("Antibacterial","Antifungal", "Antibacterial+Antifungal")),mapping=aes(x=Day,ymin=N20-se, ymax=N20+se,group=Treatment), width=0.5, size=.3,colour="black", position=position_dodge(width=0.8))+geom_bar(data=subset(SummN20_D51,Treatment %in% c("Antibacterial","Antifungal", "Antibacterial+Antifungal")), aes(x = Day, y = N20, fill=Treatment), stat ="identity", position= "dodge")+ scale_fill_manual(name="Treatment",values=c("#F0E442","#0072B2","#009E73"))+ geom_point(data=subset(SummN20_D51, Treatment %in% c("Positive Control","Negative Control")), aes(x = Day, y = N20, color=Treatment))+ geom_line(data=subset(SummN20_D51, Treatment %in% c("Positive Control","Negative Control")), aes(x = Day, y = N20,colour=Treatment, group=Treatment,linetype=Treatment),lwd=2) + scale_linetype_manual(values=c("solid","dotdash")) + geom_errorbar(data=subset(SummN20_D51,Treatment %in% c("Positive Control","Negative Control")),mapping=aes(x=Day,ymin=N20-se, ymax=N20+se,group=Treatment), width=0.2, size=.3,colour="black", position=position_dodge(width=0.01))+theme_light()+scale_color_manual(values=c("#000000","#999999"))+ theme(axis.text.x = element_text(hjust = 1,size=25),strip.text = element_text(size=25,face="bold"),axis.text.y = element_text(size=25), axis.title=element_text(size=25,face="bold"),legend.text=element_text(size=25),legend.title=element_text(size=25,face="bold"))
N20_D51plot
```
...
#write csv
write.csv(Posthoc_D51_N20flux$p.value, file = "/Volumes/S_ GANASAMU/Paper 3/Section 5/Posthoc_D51_N20flux.csv")
